...
// UNCALIBRATED: structural priors per asset class (baseline liquidity/friction/spread/
// cross-asset correlation), picked by feel to reflect "gold trades tighter than crypto"
// type intuition -- not sourced from real historical spread/correlation data, never
// backtested. computeAssetMetrics blends these with real volume/bid-ask/macro signals
// (2026-08-31), so they're a floor/prior now, not the sole driver -- but the numbers
// themselves are still design assumptions.
...
// UNCALIBRATED: same as ASSET_BASE above but per GICS sector -- design priors
// (e.g. "tech trades more liquid than utilities"), not fit against real spread/
// correlation history. computeSectorMetric still uses these as-is (not yet made
// real-data-driven the way computeAssetMetrics was).
...
// S&P long-term foreign-currency sovereign ratings -> [0-100], refreshed 2026-08-31
// with real ratings (source: S&P Global Ratings press releases / Mappr rating tracker,
// see chat for links) run through a disclosed conversion scale:
// AAA=95 AA+=92 AA=88 AA-=84 A+=80 A=76 A-=72 BBB+=68 BBB=64 BBB-=60 BB+=55 ... down to D=0.
// EE.UU. AA+ (downgraded from AAA in 2025) -> 92
const COUNTRY_CREDIT_RATINGS: Record<string, number> = {
  'EE.UU.':      92,
  'CanadÃ¡':      95, // AAA, unchanged
  'Francia':     80, // A+ (downgraded from AA in oct-2025 by S&P and Fitch)
  'Alemania':    95, // AAA, unchanged
  'Italia':      68, // BBB+ (upgraded jul-2026)
  'JapÃ³n':       80, // A+, affirmed mar-2026, unchanged
  'Reino Unido': 88, // AA, affirmed abr-2025
  'Rusia':       8,  // rating withdrawn by S&P 2022 (sanctions); last published: CC/C. Genuinely unrated since -- floor estimate, not a live rating.
};

// NOT refreshed 2026-08-31 (unlike credit ratings/CPI above): could not find current
// World Bank WGI "Political Stability" percentiles for these 8 countries from a single
// comparable source via web search -- only found EE.UU. (46.3, but from a 2023 dataset)
// and two figures for Francia/Japon on a different, non-comparable scale (BTI/OWID).
// Rather than guess the other 7, these numbers are left as they were (unverified design
// placeholders) until someone pulls the real percentiles directly from
// databank.worldbank.org (needs interactive navigation, not just search).
...
// IMF/WB capital controls â†’ [0-30, higher = more restricted]. Checked 2026-08-31:
// EE.UU./CanadÃ¡/Francia/Alemania/Italia/JapÃ³n/Reino Unido all confirmed as open capital
// accounts with no material restrictions -- the small nonzero values below for
// Francia/Alemania/Italia/JapÃ³n are fine-grained guesses within that "open" bucket,
// not specifically confirmed or refuted (real IMF AREAER country-by-country detail
// wasn't pulled). Rusia's value below IS updated with sourced current detail.
const COUNTRY_CAPITAL_CONTROLS: Record<string, number> = {
  ...
  'JapÃ³n':       3,
  'Reino Unido': 0,
  'Rusia':       18, // real, substantial controls remain (FX cash withdrawal cap ~$10k on pre-2022 accounts through 9-mar-2026; transfers still restricted for "unfriendly" countries through at least 7-jun-2026) but partially eased for "friendly"-country transfers since dic-2025 -- down from 22, not fully lifted
};

// UNCALIBRATED estimate, not sourced from a specific broker/exchange fee schedule
// (same caveat as gravityEngine.ts's COUNTRY_TX_COSTS -- these two tables should
// probably be the same table someday, they currently aren't).
...
// CPI annual, refreshed 2026-08-31 with real jul-2026 readings [%]
// (sources: US BLS/StatCan/Eurostat/ONS/Trading Economics/Bank of Russia, see chat)
const COUNTRY_CPI: Record<string, number> = {
  ...
  'JapÃ³n':       1.9,
  'Reino Unido': 2.9,
  'Rusia':       6.0, // BCR (banco central) proyecta 6-8% para el año completo 2026
};

// UNCALIBRATED DESIGN JUDGMENT, not a #5-refreshable external stat: unlike CPI/credit
// ratings below, "how dominant is tech in Germany's economy on a 0-100 scale" has no
// single official published number to look up -- these are the author's structural
// judgment calls (informed by general knowledge of each economy's makeup), not sourced
// from e.g. sector weights in each country's benchmark index. Treat as illustrative,
// not measured.
...
// UNCALIBRATED -- same caveat as gravityEngine.ts's REGIME_WEIGHTS (design heuristic,
// never fit/backtested against realized returns).
...
function computeSectorFlowsCalibrated(
  ...
  for (const f of nodes) {
    for (const t of nodes) {
      ...
      // Same fragility class fixed in computeFlows() on 2026-08-31: an absolute cutoff
      // here is redundant with the topN slice below and silently empties the result if
      // the masa/distancia scale ever shifts. Only drop non-positive pairs.
...
// UNCALIBRATED: every tilt value below (+12, -8, +15, etc.) is a textbook-macro
// judgment call (e.g. "war -> gold up"), not fit against how markets actually moved
// after similar past headlines. See the 2026-08-31 audit's backtesting section --
// news-flow rules specifically can't be backtested with the current toolchain
// (Google News RSS has no historical archive), so these stay unvalidated until a
// paid news-archive source is added.
...
export async function computeLiveSnapshot() {
  const assetSyms          = Object.values(ASSET_SYMBOLS);
    ...
    const newsFlowSignal: Record<string, { tilt: number; reasons: string[] }> = {
      ...
      ...newsFlow.sectorTilts,
        ...
        return {
      regime, vix, us10y, gravityCenters, metrics, newsHeadlines, assetPressures,
      rotationSignal: rotationResult.signal, countrySectorData, newsFlowSignal,
      newsFlowFirings: newsFlow.firings, weights, flows, usSectorNodes, usSectorFlows,
      priceRegime, newsSentimentScore, newsSentiment,
    };
  }
