 ...
 * "graduada" del filtro de confirmacion de tendencia: SMA10 sola mejoro
 * Validacion/Live pero destruyo Backtest (retrasa TODA salida por igual,
 * incluso cuando el ICD se derrumba fuerte -- en 2018-2023 hubo crisis
 * reales, ahi salir rapido protegia capital).
 *
 * Variante: sale INMEDIATO si el ICD cae por debajo de un umbral fuerte
 * (reversal real, sin esperar nada) -- SOLO exige confirmacion de precio
 * (close < SMA_N) cuando el ICD cruza a negativo de forma leve (ruido
 * tipo XLE, no reversal). Objetivo: quedarse con la mejora de
 * Validacion/Live sin volver a pagar el costo completo en Backtest.
 *
 * Correr con:
 *   node --experimental-strip-types scripts/testGradedExitM2.mjs
 */
...
const SMA_WINDOW = 10; // el que mejor rindio en la corrida anterior
const STRONG_THRESHOLDS = [-0.3, -0.5, -0.75];
...
/**
 * Sale inmediato si ICD < strongThreshold (reversal fuerte, sin esperar
 * nada). Si 0 > ICD >= strongThreshold (cruce leve a negativo), exige
 * ADEMAS close < SMA_N para salir -- si el precio sigue firme, aguanta.
 */
function simulateIcdExitGraded(sectorMetrics, lookbackDays, smaWindow, strongThreshold) {
  ...
  for (let i = lookbackDays; i < dates.length; i++) {
    ...
    if (position) {
      ...
      if (icdNow !== null && icdNow !== undefined) {
        if (icdNow < strongThreshold) {
          shouldExit = true; // reversal fuerte, sale sin esperar
        } else if (icdNow < 0) {
          const smaNow = smaByTicker[position.ticker].get(date);
          if (smaNow !== undefined && closeNow !== undefined && closeNow < smaNow) shouldExit = true;
        }
      }
      if (shouldExit && closeNow !== undefined) {
...
async function main() {
  ...
  for (const [label, start, end] of [
    ...
    for (const thr of STRONG_THRESHOLDS) {
      const curveGraded = simulateIcdExitGraded(win, LOOKBACK_DAYS, SMA_WINDOW, thr);
      console.log(`  M2 graduado (fuerte<${thr}):  ` + fmt(computeStrategyStats(curveGraded), curveGraded));
    }
