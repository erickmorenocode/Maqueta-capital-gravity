 ...
 * SH (ProShares Short S&P500, -1x SIN apalancar) al universo de M2.
 * Motivo: correlacion vs SPY por CONSTRUCCION ~-1.0 -- ataca directo el
 * hallazgo de la ventana Live/crash COVID (cuando todo cae correlacionado
 * junto, M2 no tiene a donde rotar) dandole el lado opuesto exacto del
 * trade. -1x sin apalancar (no SDS/SPXU) para evitar decaimiento por
 * rebalanceo diario que tienen los inversos apalancados.
 *
 * LIMITACION CONOCIDA (deliberada, solo para ver si vale la pena antes
 * de invertir en parsear EDGAR real): SH usa market cap/shares ACTUALES
 * (hoy) aplicados constante a toda la serie -- mismo approach rapido que
 * se uso para TLT/EWY/UUP/FXE (negativo/mixto, dos por escala chica) y
 * DBMF (unico positivo en Backtest hasta ahora). AUM de SH es mayor que
 * las divisas probadas, menos riesgo de ese problema de escala especifico.
 *
 * Correr con:
 *   node --experimental-strip-types scripts/testShUniverse.mjs
 */
...
async function main() {
  const allTickers = [...SECTOR_ETFS, 'SH', 'SPY'];
  ...
  if (!uupStatic) {
    console.log('No se pudo obtener static info de SH, abortando.');
    return;
  }
  console.log('SH static info (hoy):', uupStatic);

  const historicalSize = JSON.parse(readFileSync(path.join(__dirname, '../src/lib/densityLab/historicalSize.json'), 'utf8'));
  const sizeSeriesWithUup = { ...historicalSize, SH: [{ date: '2018-01-01', marketCap: uupStatic.marketCap, sharesOutstanding: uupStatic.sharesOutstanding }] };

  const expandedBars = {};
  for (const t of [...SECTOR_ETFS, 'SH']) if (priceBars[t]) expandedBars[t] = priceBars[t];
  ...
  for (const [label, start, end] of [
    ...
    console.log('  M2 actual (13, sin SH):   ' + fmt(computeStrategyStats(curveSectorOnly), curveSectorOnly));
    ...
    console.log('  M2 + SH (14, dato hoy):   ' + fmt(computeStrategyStats(curveExpanded), curveExpanded));
